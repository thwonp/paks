...
enum
{
    ...
    BOX_alac    = FOUR_CHAR_INT( 'a', 'l', 'a', 'c' ),//AppleLosslessAudioCodec
...
static unsigned int mp4_get_bits(bit_reader_t *bs, int n)
{
...
#define GetBits(n) mp4_get_bits(bs, n)
...
static int ue_bits(bit_reader_t *bs)
{
    int clz;
    int val;
    for (clz = 0; !mp4_get_bits(bs, 1); clz++) {}
    //mp4_get_bits(bs, clz + 1);
    val = (1 << clz) - 1 + (clz ? mp4_get_bits(bs, clz) : 0);
    return val;
}
...
broken_android_meta_hack:
        ...
        case BOX_covr:
            ...
            // Structure: size(4) + 'data'(4) + type(4) + reserved(4) + image_data
            if (payload_bytes > 16 && !mp4->tag.cover)
            {
                SKIP(16);  // Skip data atom header
                payload_bytes -= 16;
                unsigned int img_size = (unsigned int)payload_bytes;
                mp4->tag.cover = (unsigned char*)malloc(img_size);
                if (mp4->tag.cover)
                {
                    unsigned int i;
                    for (i = 0; i < img_size && !eof_flag; i++)
                    {
                        mp4->tag.cover[i] = minimp4_read(mp4, 1, &eof_flag);
                    }
                    if (!eof_flag)
                    {
                        mp4->tag.cover_size = img_size;
                    }
                    else
                    {
                        free(mp4->tag.cover);
        ...
        case BOX_mp4a:
        case BOX_alac:  // Apple Lossless uses same audio sample entry structure
            if (!tr)
            {
                ERROR("broken file structure!");
            }
